...
- **Design system (Precision Instrument):** dark `--ink-900 #0E1116` base; **one** accent `--signal #38E0C8` reserved exclusively for "data flowing"; semantics only for amber (warning) / vermilion (delete). Faces: IBM Plex Sans (UI) + IBM Plex Mono (paths, permission bits, sizes, telemetry) via Google Fonts. **No inline color/hex in components** — every value comes from CSS variables in `src/styles/tokens.css`. No numbered markers. `prefers-reduced-motion` freezes the flow animation. Built with the `frontend-design` skill's direction.
...
│   ├── src/components/{Toolbar,FilePanel,FileEntryRow,PathBreadcrumb,TransferTelemetry,ConnectDialog,EmptyState}.tsx
...
## Task 9: Frontend scaffolding — install deps, generate bindings, design system, base App

**Goal:** Working frontend dev server that builds; bindings generated from Go services; the Precision Instrument design system (tokens + fonts) in place; a top-level App shell.

**Files:**
- Create: `frontend/src/styles/tokens.css`, `frontend/src/styles/base.css`
- Modify: `frontend/src/App.tsx`, `frontend/src/main.tsx`, `frontend/index.html`, `frontend/src/style.css`
- Generate: `frontend/bindings/`

**Interfaces:**
- Produces: importable generated binding functions under `frontend/bindings/androidfs/`; CSS variables consumed by all components in Task 11.
...
Create `frontend/src/styles/tokens.css`. These are the ONLY color/type values in the whole app; components reference them as `var(--name)`.
```css
:root {
  /* ===== Palette — Precision Instrument ===== */
  --ink-900: #0E1116;   /* window base (near-black, cool) */
  --ink-800: #161A21;   /* panel surface */
  --ink-700: #1F252E;   /* input / hover */
  --ink-600: #2A313B;   /* borders */
  --mist:    #8B95A5;   /* secondary text (cool grey) */
  --frost:   #C7CED9;   /* primary text */
  --signal:  #38E0C8;   /* ONE accent: data flowing — only this */
  --amber:   #E8A33D;   /* warning (e.g. unauthorized) */
  --vermilion: #E5564E; /* danger (delete only) */
...
- [ ] **Step 4: Create base styles + reduced-motion**
...
/* Monospace is earned: paths, permission bits, sizes, telemetry */
.mono {
  font-family: var(--font-mono);
  font-variant-numeric: tabular-nums;
}

/* Keyboard focus is visible — part of the quality floor */
:focus-visible {
...
button:hover:not(:disabled) { background: var(--ink-600); }
button:disabled { opacity: 0.4; cursor: not-allowed; }

/* Signature accent button — used sparingly (e.g. primary connect) */
button.primary {
...
/* Flow animation freezes when user asks for reduced motion */
...
- [ ] **Step 5: Load fonts via `index.html`**
...
- [ ] **Step 6: Wire styles into `style.css`**

Overwrite `frontend/src/style.css` (single entry imported by `main.tsx`):
```css
@import "./styles/tokens.css";
@import "./styles/base.css";
```

- [ ] **Step 7: Replace `App.tsx` with shell (full UI in Task 11)**
...
```
(Apply these class names by appending to `frontend/src/style.css` after the imports, so the shell is already styled. Task 11 replaces the shell body with the full UI.)

Append to `frontend/src/style.css`:
```css
.app-shell { display: flex; flex-direction: column; height: 100%; padding: var(--space-6); gap: var(--space-2); }
.app-title { font-size: var(--fs-lg); font-weight: var(--fw-semibold); margin: 0; letter-spacing: 0.02em; }
.muted { color: var(--mist); font-size: var(--fs-sm); margin: 0; }
ul { list-style: none; padding: 0; margin: var(--space-2) 0 0; display: flex; flex-direction: column; gap: var(--space-1); }
li { color: var(--frost); font-size: var(--fs-sm); }
```

- [ ] **Step 8: Make `main.tsx` import consistent (default App)**
...
Expected: `frontend/dist` rebuilt without TS errors; no missing-font fallback at build time (fonts load at runtime).

- [ ] **Step 10: Commit**

```bash
git add -A
git commit -m "feat(frontend): design system tokens, base styles, fonts, device shell"
```
...
## Task 11: Frontend components + dual-pane UI (Precision Instrument design)

**Goal:** Full dual-pane file manager UI, every color/type value drawn from the token system (no inline hex). The mid-seam transfer telemetry is the signature element.

**Files:**
- Create: `frontend/src/components/{Toolbar,FilePanel,FileEntryRow,PathBreadcrumb,TransferTelemetry,ConnectDialog,EmptyState}.tsx`
- Modify: `frontend/src/App.tsx`, `frontend/src/style.css`

**Design rules for this task (enforced):** no `style={{ color: '#...' }}` anywhere. Monospace (`.mono`) only on technical data — paths, permission bits, sizes, telemetry. `--signal` only when data is flowing. No numbered markers.
...
- [ ] **Step 2: Create `PathBreadcrumb.tsx` (editable free-path input)**
...
- [ ] **Step 4: Create `TransferTelemetry.tsx` — the signature element**

The mid-seam telemetry block. When a task is active it lights up `--signal` and shows live readouts (rate / bytes·total / eta). This is the one memorable thing; everything else stays disciplined.
      ...
      {tasks.map(t => {
        ...
        return (
          <div key={t.id} className={['telem-row', active ? 'telem-active' : ''].join(' ')}>
            <span className="telem-dir mono">{t.direction === 'push' ? '↑ push' : '↓ pull'}</span>
            <span className="telem-name">{t.file_name}</span>
            <span className="telem-rate mono">
              {active ? `${(t.speed / 1024 / 1024).toFixed(1)} MB/s` : t.state}
            </span>
...
```
> The single inline `width` on the fill is allowed — it encodes live data, not a color/type choice. All color and type come from classes below.

- [ ] **Step 5: Create `ConnectDialog.tsx` and `EmptyState.tsx`**
...
- [ ] **Step 7: Rewrite `App.tsx` to compose the dual-pane layout**
...
- [ ] **Step 8: Add all component classes to `style.css`**

Append to `frontend/src/style.css` (after the `@import` lines and shell styles from Task 9). Every value is a token — no hex literals.
```css
/* ===== App shell ===== */
.app-root { display: flex; flex-direction: column; height: 100%; background: var(--ink-900); }
...
.brand { font-weight: var(--fw-semibold); font-size: var(--fs-lg); letter-spacing: 0.04em; }
.device-select { min-width: 240px; }
.toolbar-spacer { flex: 1; }
.warn { color: var(--amber); font-size: var(--fs-xs); }

/* ===== Dual panes + seam ===== */
.panes { flex: 1; display: flex; min-height: 0; }
.seam {
  width: 1px;
  background: linear-gradient(var(--ink-600), var(--ink-600));
}
...
.panel-title { font-weight: var(--fw-medium); font-size: var(--fs-sm); color: var(--mist); width: 48px; }
.badge { font-size: var(--fs-xs); color: var(--mist); }
.panel-body { flex: 1; overflow: auto; padding: var(--space-2) var(--space-3); }
.panel-error { color: var(--vermilion); padding: var(--space-3); font-size: var(--fs-sm); }
.panel-empty { padding: var(--space-6); text-align: center; }
...
.row-selected { background: var(--ink-700); }
.row-icon { width: 16px; color: var(--mist); font-size: var(--fs-sm); }
.row-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--frost); }
.row-link { color: var(--mist); font-size: var(--fs-xs); }
.row-size { width: 92px; text-align: right; color: var(--mist); font-size: var(--fs-xs); }

/* ===== Path breadcrumb ===== */
.crumbs { flex: 1; cursor: text; color: var(--mist); font-size: var(--fs-sm); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.crumb { color: var(--signal); cursor: pointer; }
.crumb-sep { margin: 0 2px; color: var(--ink-600); }
.path-input { flex: 1; }

/* ===== Telemetry — the signature ===== */
.telemetry {
...
.telem-active { background: var(--ink-700); }
.telem-dir { width: 64px; color: var(--mist); font-size: var(--fs-xs); }
.telem-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: var(--fs-sm); }
.telem-rate { width: 110px; text-align: right; font-size: var(--fs-xs); color: var(--mist); }
.telem-active .telem-rate { color: var(--signal); }
.telem-track {
  width: 140px; height: 4px; border-radius: 2px;
  background: var(--ink-900); overflow: hidden;
}
.telem-fill { height: 100%; background: var(--mist); }
/* signal lights up ONLY when data is flowing */
.telem-flow {
  background: var(--signal);
...
.telem-pct { width: 48px; text-align: right; font-size: var(--fs-xs); color: var(--frost); }
.telem-cancel { padding: 2px var(--space-2); font-size: var(--fs-xs); }
...
.empty-msg { color: var(--mist); font-size: var(--fs-md); max-width: 360px; text-align: center; }
...
.dialog-title { margin: 0 0 var(--space-4); font-size: var(--fs-lg); font-weight: var(--fw-semibold); }
.dialog-input { width: 100%; }
.dialog-error { color: var(--vermilion); margin-top: var(--space-2); font-size: var(--fs-sm); }
.dialog-actions { display: flex; justify-content: flex-end; gap: var(--space-2); margin-top: var(--space-4); }

/* ===== Utility ===== */
.muted { color: var(--mist); }
```
> The only rgba literals is in `.overlay`/`.dialog` for shadow/veil — these are not palette colors, they are opacity over `--ink`, which is acceptable and standard. No palette hex outside `tokens.css`.

- [ ] **Step 9: Build and verify**

Run: `cd frontend && npm run build && cd ..`
Expected: build succeeds with no TS errors. No `style={{ color:` occurrences remain in components (the single live-data `width` on the telemetry fill is permitted).

- [ ] **Step 10: Commit**

```bash
git add -A
git commit -m "feat(frontend): precision-instrument dual-pane UI with telemetry signature"
```
